...
const Customers = () => {
    ...
    const exportToExcel = () => {
        const csvContent = [
            ['Order ID', 'Customer Name', 'Email', 'Phone', 'Service', 'Amount', 'Status', 'Payment Status', 'Date'],
            ...orders.map(order => [
                order.id, order.customerName, order.email, order.phone,
                order.service, order.amount, order.status, order.paymentStatus, order.date
            ])
        ]
            .map(row => row.join(','))
            .join('\n');
    ...
    return (
        <div style={{ marginTop: '80px' }}>
            ...
            <div className="card mb-4">
                <div className="card-body">
                    <h5 className='mb-2'>Export Orders to Excel</h5>
                    <p>Download all order data in CSV format for analysis.</p>
                    <button
                        className="btn text-white mb-2 mt-3"
